...
def animate_frame(elementi, vozlisca, eig_vals, eig_vecs, mode_idx, SCALE):
    """
    animate beam deforamations - without applying rotation. Comes later.
    ...
    def get_deformed(mode_idx, t):
        """Return deformed node coordinates for a sinusoidal oscillation at time *t*."""
        # Extract only translational DOFs (ux, uy), skip rotation (θz)
        # DOF order per node: [ux, uy, θ] → indices 0::3, 1::3
        vec = eig_vecs[:, mode_idx]
        phi = np.column_stack([vec[0::3], vec[1::3]])  # (n_nodes, 2)
        # Normalise so the largest component equals SCALE
        max_displacement = np.max(np.abs(phi))
        if max_displacement > 0:
            phi = phi / max_displacement * SCALE
