...
class GlobalAssembly:
    ...
    Uporablja vektoriziran pristop (brez for-zank) in razpršen (COO) zapis,
    kar zagotavlja visoko računsko učinkovitost za velike mreže.
    """
    ...
    def assemble_matrices(self):
        ...
        # 1. Ekstrakcija lastnosti za vse elemente
        E = np.zeros(Ne)
        G = np.zeros(Ne)
        A = np.zeros(Ne)
        Iy = np.zeros(Ne)
        ...
        v_ref = np.zeros(shape=(Ne, 3)) # shranimo vektor orientacije
        ...
        # 2. Izračun geometrijskih vektorjev
        ...
        # 3. Vektorizirana generacija lokalnih matrik 
        ...
        # --- Bending Z -> v displacements and rz rotations ---
        # Vrednosti matrik ustrezajo blokom iz frame3D.py:
        k_bz_12 = 12 * E * Iz / L**3
        k_bz_6  = 6 * E * Iz / L**2
        k_bz_4  = 4 * E * Iz / L
        k_bz_2  = 2 * E * Iz / L
        ...
        # --- Bending Y -> w displacements and ry rotations ---
        # Uporaba enake predloge kot v starem okviru.
        k_by_12 = 12 * E * Iy / L**3
        k_by_6  = 6 * E * Iy / L**2
        k_by_4  = 4 * E * Iy / L
        k_by_2  = 2 * E * Iy / L
        ...
        # --- Correction for Truss3D elements ---
        # Palice imajo samo translacijsko maso, nimajo rotacijske vztrajnosti.
        is_truss = np.array([e.elem_type == 'Truss3D' for e in self.mesh.elements])
        if np.any(is_truss):
            # Ničenje vseh masnih vnosov za palice
            M_local[is_truss, :, :] = 0.0
            
            # Ponovni izračun translacijske mase
            m_ax_truss = (rho * A * L / 6.0)[is_truss]
            
            # X direction (DOFs 0, 6)
        ...
        # --- Damping Rayleigh Matrix ---
        # C = alpha * M + beta * K
        C_local = alpha[:, None, None] * M_local + beta[:, None, None] * K_local

        # 4. Globalne rotacije preko Einsteinove sumacije
        x = d / L[:, None]
        ...
        # Uporaba [T]^T [K_local] [T] po blokih
        # Hitro množenje vseh elementov z einsum
        ...
        # 5. Hitra sinteza Scipy.Sparse COO
        # Ustvarjanje globalnih mapiranj indeksov.
...
class Solvers:
    """
    Matematični reševalniki za strukturno analizo (modalna, harmonska in tranzientna analiza).
    """
    def __init__(self, K, M, C_mat, dof_manager):
        # Matrike vedno stisnemo v reševalniku za hitrejše klice
        self.K = K.tocsr()
        self.M = M.tocsr()
    ...
    def project_matrix(self, matrix):
        ...
        # Razpršenost omogoča nizko porabo pomnilnika
    ...
    def solve_eigen(self):
        ...
        # Gosti posplošeni reševalec lastnih vrednosti
        eig_vals, eig_vecs_red = spla.eigh(K_red_dense, M_red_dense)
        
        # Preslikava fizikalnih lastnih vektorjev
        L = self.dof_manager.get_L_matrix()
        eig_vecs_full = L @ eig_vecs_red
        
        # eigvals moramo se koreniti in deliti z 2 pi
        return eig_vals, eig_vecs_full
        
    
    # Harmonska analiza - ena vzbujevalna sila - spremeni kodo da bo enako kot smo delali na predavanjih
    def solve_harmonic(self, F_glob, omega_sweep):
            ...
            # Če C_mat obstaja, ga projiciraj, sicer uporabi ničle.
            if self.C_mat is not None:
                C_red_dense = self.project_matrix(self.C_mat)
            else:
                C_red_dense = np.zeros_like(K_red_dense)

            # 2. Problem lastnih vrednosti (avtomatska normacija)
            # eig_vals = omega_n^2
            eig_vals, mod_mat = spla.eigh(K_red_dense, M_red_dense)
            
            # 3. Projekcija sile in dušenja v modalni prostor
            # F_modal shape: (n_modes,)
            F_modal = mod_mat.T @ L.T @ F_glob 
            
            # Diagonala C_modal predstavlja dušenje
            # c_m = 2 * zeta_m * omega_m
            C_modal_matrix = mod_mat.T @ C_red_dense @ mod_mat
            c_m = np.diag(C_modal_matrix) 

            # 4. Izračun modalnega odziva (vektorizirano)
            # Preoblikovanje polj za pravilno množenje:
            # Vrstice = oblike, Stolpci = frekvenčni koraki
            
            w = omega_sweep[None, :]       # Shape: (1, n_freqs)
            wn2 = eig_vals[:, None]        # Shape: (n_modes, 1)  (Already squared)
            c = c_m[:, None]               # Shape: (n_modes, 1)
            F_m = F_modal[:, None]         # Shape: (n_modes, 1)
            # H je odziv modalnih koordinat (eta)
            H = F_m / (-w**2 + 1j * w * c + wn2)

            # 5. Prehod nazaj v fizikalni prostor
            U_red = mod_mat @ H            # Shape: (n_red_dofs, n_freqs)
            U_full = L @ U_red             # Shape: (n_full_dofs, n_freqs)
    ...
    def solve_integration_ivp(self, force_func, x0, dx0, t_span, t_eval, method='Radau'):
        """
        Časovna integracija (tranzientna analiza) preko State-Space prostora in reševalnika solve_ivp.
        ...
        N = M_red.shape[0]  # Število reduciranih prostostnih stopenj
        M_inv = np.linalg.inv(M_red) # Inverz masne matrike

        # 2. Izračun matrike stanj (D)
        D = np.block([
            [-M_inv @ C_red, -M_inv @ K_red],
            [np.eye(N), np.zeros((N, N))]
        ])

        # 3. Definicija odvoda za solve_ivp
        def int_fun(t, y):
            # Klic funkcije za globalno silo in projekcija
            F_glob = force_func(t)
            F_red = L.T @ F_glob
            
            # Izgradnja vektorja h
            h = np.block([
                M_inv @ F_red,
                np.zeros(N)
            ])
            
            # Enačba stanj: y_dot = D * y + h
            return D @ y + h

        # 4. Začetni pogoji
        y0 = np.concatenate([L.T@dx0, L.T@x0])

        # 5. Reševanje diferencialne enačbe (ODE)
        print("Solving Time Integration via solve_ivp. This may take a moment...")
        res = integrate.solve_ivp(fun=int_fun, t_span=t_span, y0=y0, method=method, t_eval=t_eval)

        # 6. Ekstrakcija rezultatov
        # 'res.y' has shape (2N, num_time_steps). 
        # Zgornja polovica so hitrosti, spodnja pomiki.
        q_dot_red, q_red = np.split(res.y,2)
        
        # 7. Prehod v globalni prostor
        U_full = L @ q_red
    ...
    def solve_integration_newmark(self, force_array: np.ndarray, dt: float, x_0: np.ndarray, v_0: np.ndarray, gamma: float, beta: float):
        ...
        # 1. REDUKCIJA SISTEMA (Zelo pomembno zaradi robnih pogojev)
        L = self.dof_manager.get_L_matrix()
        K_red = self.project_matrix(self.K)
        M_red = self.project_matrix(self.M)
        C_red = self.project_matrix(self.C_mat) if self.C_mat is not None else np.zeros_like(K_red)

        # Projekcija sil in začetnih pogojev
        ...
        # PRED-FAKTORIZACIJA (Trik za 100x hitrejše reševanje)
        lu, piv = spla.lu_factor(K_eff)

        # 4. INICIALIZACIJA MATRIK ZA REZULTATE
        ...
        # 5. ALGORITEM (Zanka po času)
        for i in tq.tqdm(range(n_steps - 1)):
            
            # Efektivna sila
            p = F_red[:, i+1] + a1 @ u[:, i] + a2 @ v[:, i] + a3 @ a[:, i]
            
            # Reševanje pomikov (Zelo hitro zaradi pre-faktorizacije)
            u[:, i+1] = spla.lu_solve((lu, piv), p)
            
            # Reševanje hitrosti (Dodane manjkajoče zvezdice * za množenje)
            v[:, i+1] = (gamma / (beta * dt)) * (u[:, i+1] - u[:, i]) + (1 - gamma / beta) * v[:, i] + dt * (1 - gamma / (2 * beta)) * a[:, i]
            
            # Reševanje pospeškov (Direktna algebrska formula - hitreje kot matrix solve)
            a[:, i+1] = (1 / (beta * dt**2)) * (u[:, i+1] - u[:, i]) - (1 / (beta * dt)) * v[:, i] - (1 / (2 * beta) - 1) * a[:, i]
        
        # 6. VRNITEV V GLOBALNI PROSTOR
        U_full = L @ u
        V_full = L @ v
        A_full = L @ a
